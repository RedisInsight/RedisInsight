...
const staticDir =
  process.env.BUILD_TYPE === 'ELECTRON' && process['resourcesPath']
...
export default {
  ...
  sockets: {
    cors: process.env.SOCKETS_CORS
      ? process.env.SOCKETS_CORS === 'true'
      : false,
    serveClient: process.env.SOCKETS_SERVE_CLIENT
      ? process.env.SOCKETS_SERVE_CLIENT === 'true'
      : false,
  },
  db: {
  ...
  redis_clients: {
    idleSyncInterval:
      parseInt(process.env.CLIENTS_IDLE_SYNC_INTERVAL, 10) || 1000 * 60 * 60, // 1hr
    maxIdleThreshold:
      parseInt(process.env.CLIENTS_MAX_IDLE_THRESHOLD, 10) || 1000 * 60 * 60, // 1hr
    retryTimes: parseInt(process.env.CLIENTS_RETRY_TIMES, 10) || 5,
    retryDelay: parseInt(process.env.CLIENTS_RETRY_DELAY, 10) || 500,
    maxRetriesPerRequest:
      parseInt(process.env.CLIENTS_MAX_RETRIES_PER_REQUEST, 10) || 1,
  },
  redis_scan: {
  ...
  redis_cli: {
    unsupportedCommands: JSON.parse(
      process.env.CLI_UNSUPPORTED_COMMANDS || '[]',
    ),
  },
  profiler: {
    logFileIdleThreshold:
      parseInt(process.env.PROFILER_LOG_FILE_IDLE_THRESHOLD, 10) || 1000 * 60, // 1min
  },
  analytics: {
  ...
  logger: {
    ...
    files: process.env.FILES_LOGGER
      ? process.env.FILES_LOGGER === 'true'
      : true, // enabled by default
    omitSensitiveData: process.env.LOGGER_OMIT_DATA
      ? process.env.LOGGER_OMIT_DATA === 'true'
      : true,
    pipelineSummaryLimit:
      parseInt(process.env.LOGGER_PIPELINE_SUMMARY_LIMIT, 10) || 5,
  },
  plugins: {
    stateMaxSize:
      parseInt(process.env.PLUGIN_STATE_MAX_SIZE, 10) || 1024 * 1024,
  },
  guides: {
    updateUrl:
      process.env.GUIDES_UPDATE_URL ||
  ...
  tutorials: {
    updateUrl:
      process.env.TUTORIALS_UPDATE_URL ||
  ...
  content: {
    updateUrl:
      process.env.CONTENT_UPDATE_URL ||
  ...
  notifications: {
    updateUrl:
      process.env.NOTIFICATION_DEV_PATH ||

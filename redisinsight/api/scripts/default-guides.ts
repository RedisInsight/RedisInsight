...
import { Config, get } from '../src/utils/config';

const PATH_CONFIG = get('dir_path');
const GUIDES_CONFIG = get('guides') as Config['guides'];

const archiveUrl = new URL(
  path.join(GUIDES_CONFIG.updateUrl, GUIDES_CONFIG.zip),
).toString();

const buildInfoUrl = new URL(
  path.join(GUIDES_CONFIG.updateUrl, GUIDES_CONFIG.buildInfo),
).toString();
...
async function init() {
  try {
    ...
    console.error(
      'Something went wrong trying to get default guides archive',
      e,
    );
    process.exit(1);
  }

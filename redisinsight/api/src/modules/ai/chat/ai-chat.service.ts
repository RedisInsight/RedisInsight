...
export class AiChatService {
  constructor(
    private readonly convAiProvider: ConvAiProvider,
  ) {
  }
  ...
  async postMessage(sessionMetadata: SessionMetadata, chatId: string, dto: SendAiChatMessageDto) {
    return this.convAiProvider.postMessage(sessionMetadata, chatId, dto.content);
  }
  ...
  async delete(sessionMetadata: SessionMetadata, chatId: string): Promise<void> {

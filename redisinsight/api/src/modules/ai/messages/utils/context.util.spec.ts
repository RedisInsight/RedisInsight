import {
  mockAiFullDbContext,
  mockAiGetDescriptionTopValuesReply,
  mockAiGetPriceTopValuesReply, mockAiGetTypeTopValuesReply,
  mockAiHScanReply,
  mockAiIndex, mockAiIndexContext,
  mockAiIndexInfoObject, mockAiIndexInfoReply,
  mockAiJsonReply, mockAiSchema, mockAiSchemaForHash,
  ...
  describe('getDocumentsSchema', () => {
    beforeEach(() => {
      ...
      when(mockStandaloneRedisClient.sendCommand)
        ...
        .mockResolvedValueOnce(mockAiJsonReply);
      when(mockStandaloneRedisClient.sendCommand)
        .calledWith(expect.arrayContaining(['HSCAN']), expect.anything())
        .mockResolvedValueOnce(mockAiHScanReply);
    });

    it('Should get document schema for json type', async () => {
      expect(await getDocumentsSchema(mockStandaloneRedisClient, mockAiIndex, mockAiIndexInfoObject))
        .toEqual(mockAiSchema);
    });
  ...
  describe('getIndexContext', () => {
    beforeEach(() => {
      ...
      when(mockStandaloneRedisClient.sendCommand)
        ...
        .mockResolvedValue(mockAiGetPriceTopValuesReply)
        .calledWith(expect.arrayContaining(['FT.AGGREGATE', '@description']), expect.anything())
        .mockResolvedValue(mockAiGetDescriptionTopValuesReply)
        .calledWith(expect.arrayContaining(['FT.AGGREGATE', '@type']), expect.anything())
        .mockResolvedValue(mockAiGetTypeTopValuesReply);
      when(mockStandaloneRedisClient.sendCommand)
        .calledWith(expect.arrayContaining(['FT.INFO']), expect.anything())
        .mockResolvedValue(mockAiIndexInfoReply);
      when(mockStandaloneRedisClient.sendCommand)
        .calledWith(expect.arrayContaining(['json.get']), expect.anything())
        .mockResolvedValue(mockAiJsonReply);
      when(mockStandaloneRedisClient.sendCommand)
        .calledWith(expect.arrayContaining(['HSCAN']), expect.anything())
        .mockResolvedValue(mockAiHScanReply);
    });

    it('Should get index context', async () => {
      expect(await getIndexContext(mockStandaloneRedisClient, mockAiIndex))
        .toEqual(mockAiIndexContext);
    });
  });

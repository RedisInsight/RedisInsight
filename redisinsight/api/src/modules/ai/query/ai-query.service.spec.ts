...
import { AiQueryServerErrors, AiQueryWsEvents } from 'src/modules/ai/query/models';
...
describe('AiQueryService', () => {
  ...
  describe('stream', () => {
    ...
    it('should stream an answer', async () => {
      serverSocket.once(AiQueryWsEvents.STREAM, (_content, _context, _history, _opts, cb) => {
        serverSocket.emit(AiQueryWsEvents.REPLY_CHUNK, mockAiQueryAiResponse.content);
        cb({ status: 'ok' });
      });
    ...
    it('should not fail in case of empty ack', async () => {
      ...
      serverSocket.once(AiQueryWsEvents.STREAM, (_content, _context, _history, _opts, cb) => {
        serverSocket.emit(AiQueryWsEvents.TOOL_CALL, JSON.stringify(mockAiQueryAiIntermediateStep));
        serverSocket.emit(AiQueryWsEvents.TOOL_REPLY, JSON.stringify(mockAiQueryAiIntermediateStep2));
        serverSocket.emitWithAck(AiQueryWsEvents.GET_INDEX, mockAiQueryIndex)
          .then((indexContext) => {
            expect(indexContext).toEqual(mockAiQueryIndexContext);

            return serverSocket.emitWithAck(AiQueryWsEvents.RUN_QUERY, ['ft.aggregate']);
          })
          .then((queryResult) => {
    ...
    it('should calculate index and get from cache on 2nd attempt and send error on 3d attempt', async () => {
      serverSocket.once(AiQueryWsEvents.STREAM, (_content, _context, _history, _opts, cb) => {
        ...
        aiQueryContextRepository.getIndexContext.mockResolvedValueOnce({ cached: 'cached' });
        aiQueryContextRepository.getIndexContext.mockRejectedValueOnce(new Error('Something is wrong'));
        serverSocket.emitWithAck(AiQueryWsEvents.GET_INDEX, mockAiQueryIndex)
          .then((indexContext) => {
            // calculated
            expect(indexContext).toEqual(mockAiQueryIndexContext);

            return serverSocket.emitWithAck(AiQueryWsEvents.GET_INDEX, mockAiQueryIndex);
          })
          .then((indexContext) => {
            // cached
    ...
    it('should return errors for run queries', async () => {
      serverSocket.once(AiQueryWsEvents.STREAM, (_content, _context, _history, _opts, cb) => {
        ...
        serverSocket.emitWithAck(AiQueryWsEvents.RUN_QUERY, ['FLUSHALL'])
          .then((result) => {
          ...
          .then((result) => {
            ...
            return serverSocket.emitWithAck(AiQueryWsEvents.RUN_QUERY, ['FT.AGGREGATE', 'BAD ARGS']);
          })
          .then((result) => {
            // execution error
  ...
  describe('getHistory', () => {
    it('should get history', async () => {
      expect(await service.getHistory(mockSessionMetadata, mockAiQueryDatabaseId)).toEqual(mockAiQueryHistory);
    });
    ...
    it('throw CloudApiUnauthorizedException exception', async () => {
      ...
      await expect(service.getHistory(mockSessionMetadata, mockAiQueryDatabaseId)).rejects.toThrow(
  ...
  describe('clearHistory', () => {
    it('should clear history', async () => {
      expect(await service.clearHistory(mockSessionMetadata, mockAiQueryDatabaseId)).toEqual(undefined);
    });
    ...
    it('throw CloudApiUnauthorizedException exception', async () => {
      ...
      await expect(service.clearHistory(mockSessionMetadata, mockAiQueryDatabaseId)).rejects.toThrow(
        CloudApiUnauthorizedException,
      );
    });
  });

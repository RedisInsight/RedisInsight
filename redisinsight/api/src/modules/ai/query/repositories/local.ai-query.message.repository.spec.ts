...
describe('LocalAiQueryAuthProvider', () => {
  ...
  describe('cleanupDatabaseHistory', () => {
    beforeEach(() => {
      repository.createQueryBuilder().getRawMany.mockResolvedValueOnce([mockAiQueryHumanMessage.id]);
    });

    it('should get id to and remove it', async () => {
      await expect(service['cleanupDatabaseHistory'](mockAiQueryDatabaseId, mockAiQueryAccountId))
  ...
  describe('list', () => {
    ...
    it('should return list of messages', async () => {
      await expect(service.list(mockSessionMetadata, mockAiQueryDatabaseId, mockAiQueryAccountId))
        .resolves.toEqual([
          mockAiQueryHumanMessage,
          mockAiQueryAiResponse,
          mockAiQueryHumanMessage2,
  ...
  describe('clearHistory', () => {
    it('should clear history', async () => {
      await expect(service.clearHistory(mockSessionMetadata, mockAiQueryDatabaseId, mockAiQueryAccountId))
        .resolves.toEqual(undefined);
    });
  });
});

...
import {
  mockCloudSessionService, mockDefaultCloudApiHeaders, mockSessionMetadata, MockType,
} from 'src/__mocks__';
...
const getHeadersTests = [
  ...
  },
  {
    ...
    expected: { ...mockDefaultCloudApiHeaders, authorization: 'Bearer jwt-token' },
  },
  {
...
describe('CloudApiProvider', () => {
  ...
  describe('callWithAuthRetry', () => {
    it('should return result from 1st attempt', async () => {
      expect(await service.callWithAuthRetry(mockSessionMetadata.sessionId, mockedFn))
    ...
    it('should not fail when session invalidation throw an error', async () => {
      ...
      sessionService.invalidateApiSession.mockRejectedValueOnce(new Error('Unable to invalidate'));
      expect(await service.callWithAuthRetry(mockSessionMetadata.sessionId, mockedFn))
        .toEqual(mockedResult);
      expect(sessionService.invalidateApiSession).toHaveBeenCalledWith(mockSessionMetadata.sessionId);
    ...
    it('should throw an error from 1st attempt if not CloudApiUnauthorizedException (and keep session)', async () => {
      ...
      await expect(service.callWithAuthRetry(mockSessionMetadata.sessionId, mockedFn))
    ...
    it('should throw CloudApiForbiddenException error from 3rd attempt (custom attempts)', async () => {
      ...
      mockedFn.mockRejectedValueOnce(new CloudApiUnauthorizedException());
      mockedFn.mockRejectedValueOnce(new CloudApiUnauthorizedException());
      await expect(service.callWithAuthRetry(mockSessionMetadata.sessionId, mockedFn, 2))
        .rejects.toBeInstanceOf(CloudApiUnauthorizedException);

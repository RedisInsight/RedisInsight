...
describe('CloudSubscriptionCapiService', () => {
  ...
  describe('getSubscriptions', () => {
    it('successfully get cloud subscriptions', async () => {
      expect(await service.getSubscriptions(
        mockCloudCapiAuthDto,
        CloudSubscriptionType.Flexible,
      )).toEqual([mockCloudSubscription]);
    });
    it('throw CloudApiUnauthorizedException exception', async () => {
      capi.getSubscriptionsByType.mockRejectedValueOnce(new CloudApiUnauthorizedException());
  ...
  describe('getSubscription', () => {
    it('successfully get cloud subscription', async () => {
      expect(await service.getSubscription(
        mockCloudCapiAuthDto,
    ...
    it('throw CloudApiUnauthorizedException exception', async () => {
      capi.getSubscriptionByType.mockRejectedValueOnce(new CloudApiUnauthorizedException());
  ...
  describe('getSubscriptionsPlans', () => {
    it('successfully get cloud subscriptions plans', async () => {
      expect(await service.getSubscriptionsPlans(
        mockCloudCapiAuthDto,
        CloudSubscriptionType.Fixed,
      )).toEqual([mockFreeCloudSubscriptionPlan1]);
    });
    it('throw CloudApiUnauthorizedException exception', async () => {
      capi.getSubscriptionsPlansByType.mockRejectedValueOnce(new CloudApiUnauthorizedException());
      await expect(service.getSubscriptionsPlans(
        mockCloudCapiAuthDto,
        CloudSubscriptionType.Fixed,
      )).rejects.toThrow(
        CloudApiUnauthorizedException,
  ...
  describe('createFreeSubscription', () => {
    ...
    it('throw CloudApiUnauthorizedException exception', async () => {
      capi.createFreeSubscription.mockRejectedValueOnce(new CloudApiUnauthorizedException());

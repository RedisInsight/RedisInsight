...
describe('preSetupDiscoveryUtil', () => {
  ...
  beforeEach(() => {
    ...
    when(fsReadFileSpy)
      ...
      .mockResolvedValue(
        Buffer.from(mockDatabaseToImportWithCertsFromEnvsInput.caCert.certificate, 'utf8'),
      )
      .calledWith('/user.crt', 'utf8')
      .mockResolvedValue(
        Buffer.from(mockDatabaseToImportWithCertsFromEnvsInput.clientCert.certificate, 'utf8'),
      )
      .calledWith('/user.key', 'utf8')
      .mockResolvedValue(
        Buffer.from(mockDatabaseToImportWithCertsFromEnvsInput.clientCert.key, 'utf8'),
      );
  ...
  describe('scanProcessEnv', () => {
    it('should return 3 discovered env names', () => {
      ...
      expect(scanProcessEnv()).toEqual(['RI_REDIS_HOST', 'RI_REDIS_HOST_1', 'RI_REDIS_HOST_2']);
    });
  ...
  describe('populateDefaultValues', () => {
    it('should set default port and calculate database name based on host and port', () => {
      expect(populateDefaultValues({
        host: 'host',
      } as Database)).toMatchObject({
        name: 'host:6379',
      });
    });
    it('should prepare object with default values for not specified fields', () => {
      expect(populateDefaultValues(mockDatabaseToImportFromEnvsInput))
        .toEqual(mockDatabaseToImportFromEnvsPrepared);
    });
    it('should prepare object with default values and certificates', () => {
      expect(populateDefaultValues(mockDatabaseToImportWithCertsFromEnvsInput))
        .toEqual(mockDatabaseToImportWithCertsFromEnvsPrepared);
    });
    it('should prepare object with default values for not specified fields', () => {
      expect(populateDefaultValues(mockDatabaseToImportFromFileInput))
        .toEqual(mockDatabaseToImportFromFilePrepared);
    });
    it('should prepare object with default values and certificates (from file flow)', () => {
      expect(populateDefaultValues(mockDatabaseToImportWithCertsFromFileInput))
  ...
  describe('getCertificateData', () => {
    it('should get base64 certificate from env', async () => {
      ...
      await expect(getCertificateData('RI_REDIS_TLS_CA', '_1'))
        .resolves.toEqual(mockDatabaseToImportWithCertsFromEnvsInput.caCert.certificate);
  ...
  describe('prepareDatabaseFromEnvs', () => {
    ...
    it('should discover database with certs in base64 format', async () => {
      ...
      await expect(prepareDatabaseFromEnvs('RI_REDIS_HOST_1'))
        .resolves.toEqual(mockDatabaseToImportWithCertsFromEnvsPrepared);
    });
  ...
  describe('discoverEnvDatabasesToAdd', () => {
    it('should discover 2 out of 3 env databases due to validation', async () => {
      ...
      await expect(discoverEnvDatabasesToAdd())
        .resolves.toEqual([{
          ...mockDefaultDatabaseFields,
          id: '0',
          host: 'host1',
    ...
    it('should not fail in case of an error', async () => {
      ...
      scanProcessEnvSpy.mockImplementation(() => { throw new Error('Some error'); });
  ...
  describe('discoverFileDatabasesToAdd', () => {
    ...
    it('should prepare 2 out of 3 database because of validation error', async () => {
      fsReadFileSpy.mockResolvedValueOnce(Buffer.from(JSON.stringify([
        {
      ...
      await expect(discoverFileDatabasesToAdd('/databases.json'))
        .resolves.toEqual([mockDatabaseToImportFromFilePrepared, mockDatabaseToImportWithCertsFromFilePrepared]);
      expect(fsReadFileSpy).toHaveBeenCalledWith('/databases.json', 'utf8');
    });
  });
});

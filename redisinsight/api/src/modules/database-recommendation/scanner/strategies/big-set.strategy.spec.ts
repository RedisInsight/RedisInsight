...
describe('BigSetStrategy', () => {
  ...
  describe('isRecommendationReached', () => {
    it('should return false when set length < 100 000', async () => {
      expect(await strategy.isRecommendationReached(mockSetInfo)).toEqual(false);
    });

    it('should return false when not set key', async () => {
      expect(await strategy.isRecommendationReached(mockHashInfo)).toEqual(false);
    });

    it('should return true when set length > 100 000', async () => {
      expect(await strategy.isRecommendationReached(mockBigSetInfo)).toEqual(true);
    });
  });
});

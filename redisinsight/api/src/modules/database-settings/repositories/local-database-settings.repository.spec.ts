...
describe('LocalDatabaseSettingsRepository', () => {
  ...
  describe('get', () => {
    it('should return database settings entity', async () => {
      const actual = await localDbRepository.get(mockSessionMetadata, mockDatabaseId);
    ...
    it('should throw when database setting is not found', async () => {
      ...
      await expect(localDbRepository.get(mockSessionMetadata, mockDatabaseId)).rejects.toThrow(NotFoundException);
    });
  });
  describe('createOrUpdate', () => {
    it('should be able to create database settings entity', async () => {
      repository.findOneBy.mockResolvedValueOnce(null);
      const actual = await localDbRepository.createOrUpdate(mockSessionMetadata, mockDatabaseSettingsDto());
      const expected = mockDatabaseSettingsDto();
      expect(actual).toEqual(expected);
    });
  ...
  describe('delete', () => {
    it('should delete database settings entity', async () => {
      await expect(localDbRepository.delete(mockSessionMetadata, mockDatabaseId)).resolves.not.toThrow();
    });
    ...
    it('should throw when database setting is not found', async () => {
      ...
      await expect(localDbRepository.delete(mockSessionMetadata, mockDatabaseId))

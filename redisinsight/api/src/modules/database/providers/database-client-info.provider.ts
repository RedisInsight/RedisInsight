...
@Injectable()
export class DatabaseClientInfoProvider {
  constructor(
  ...
  public async determineDatabaseModules(client: RedisClient): Promise<AdditionalRedisModule[]> {
    try {
      const reply = await client.call(['module', 'list']) as string[][];
  ...
  public async determineDatabaseServer(client: RedisClient): Promise<string> {
    try {
      const reply = convertRedisInfoReplyToObject(await client.call(['info', 'server']) as string);
  ...
  public async determineDatabaseModulesUsingInfo(client: RedisClient): Promise<AdditionalRedisModule[]> {
    ...
    await Promise.all(Array.from(REDIS_MODULES_COMMANDS, async ([moduleName, commands]) => {
      try {
        let commandsInfo = await client.call(['command', 'info', ...commands]) as string[];
  ...
  public async getRedisGeneralInfo(client: RedisClient): Promise<RedisDatabaseInfoResponse> {
  ...
  private async getRedisMasterNodesGeneralInfo(client: RedisClient): Promise<RedisDatabaseInfoResponse> {
  ...
  public async getClientListInfo(client: RedisClient): Promise<any[]> {
    try {
      const clientListResponse = await client.call(['client', 'list']) as string;

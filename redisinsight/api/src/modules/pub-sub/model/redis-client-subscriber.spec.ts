...
describe('RedisClient', () => {
  ...
  beforeEach(() => {
    jest.resetAllMocks();
    redisClientSubscriber = new RedisClientSubscriber('databaseId', getRedisClientFn);
  ...
  describe('getClient', () => {
    ...
    it('should connect and return client by default', async () => {
      ...
      expect(redisClientSubscriber['status']).toEqual(RedisClientSubscriberStatus.Connected);
    });
    it('should wait until first attempt of connection finish with success', async () => {
      redisClientSubscriber.getClient().then().catch();
      expect(redisClientSubscriber['status']).toEqual(RedisClientSubscriberStatus.Connecting);
      expect(await redisClientSubscriber.getClient()).toEqual(nodeClient);
      expect(connectSpy).toHaveBeenCalledTimes(1);
      expect(redisClientSubscriber['status']).toEqual(RedisClientSubscriberStatus.Connected);
    });
    it('should wait until first attempt of connection finish with error', async () => {
      try {
        ...
        expect(connectSpy).toHaveBeenCalledTimes(1);
        expect(redisClientSubscriber['status']).toEqual(RedisClientSubscriberStatus.Error);
      }
    });
  ...
  describe('connect', () => {
    ...
    afterAll(() => { nodeClient.removeAllListeners(); });
  });

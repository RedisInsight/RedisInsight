...
export class RecommendationProvider {
  ...
  async determineLuaScriptRecommendation(
    ...
    try {
      const info = convertRedisInfoReplyToObject(
        (await redisClient.sendCommand(['info', 'memory'], {
          replyEncoding: 'utf8',
        })) as string,
      );
  ...
  async determineLogicalDatabasesRecommendation(
    ...
    try {
      const info = convertRedisInfoReplyToObject(
        (await redisClient.sendCommand(['info', 'keyspace'], {
          replyEncoding: 'utf8',
        })) as string,
      );
  ...
  async determineConnectionClientsRecommendation(
    ...
    try {
      const info = convertRedisInfoReplyToObject(
        (await redisClient.sendCommand(['info', 'clients'], {
          replyEncoding: 'utf8',
        })) as string,
      );
  ...
  async determineRedisVersionRecommendation(
    ...
    try {
      const info = convertRedisInfoReplyToObject(
        (await redisClient.sendCommand(['info', 'server'], {
          replyEncoding: 'utf8',
        })) as string,
      );

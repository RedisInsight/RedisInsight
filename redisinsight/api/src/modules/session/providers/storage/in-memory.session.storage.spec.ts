...
describe('InMemorySessionStorage', () => {
  ...
  describe('createSession', () => {
    it('Should create new session if not exists', async () => {
      ...
      expect(service['sessions'].get(mockInitSession.id)).toEqual(mockInitSession);
    });
    ...
    it('Should create new session if not exists 2 times', async () => {
      ...
      const session2 = await service.createSession({ ...mockInitSession, id: '_2' });
      ...
      expect(service['sessions'].get(mockInitSession.id)).toEqual(mockInitSession);
      expect(service['sessions'].get('_2')).toEqual({ ...mockInitSession, id: '_2' });
    });
  });
  ...
  describe('updateSessionData', () => {
    ...
    it('Should add custom data', async () => {
      ...
      const result = await service.updateSessionData(mockInitSession.id, mockSessionCustomData);
    ...
    it('Should return null if no session found and not affect existing sessions', async () => {
      ...
      expect(service['sessions'].size).toEqual(1);
      expect(await service.getSession(mockInitSession.id)).toEqual(mockInitSession);

      const result = await service.updateSessionData('_2', mockSessionCustomData);
  ...
  describe('deleteSession + getSession', () => {
    it('should delete session by id', async () => {
      service['sessions'] = new Map([[mockInitSession.id, mockInitSession], ['_2', { ...mockInitSession, id: '_2' }]]);

      expect(service['sessions'].size).toEqual(2);

      expect(await service.getSession(mockInitSession.id)).toEqual(mockInitSession);
      expect(await service.getSession('_2')).toEqual({ ...mockInitSession, id: '_2' });

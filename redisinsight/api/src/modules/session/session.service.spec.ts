...
import {
  mockInitSession, mockSessionCustomData, mockSessionProvider, MockType,
} from 'src/__mocks__';
...
describe('SessionService', () => {
  ...
  describe('getSession', () => {
    it('Should get session by id', async () => {
      ...
      expect(sessionProvider.getSession).toHaveBeenCalledWith(mockInitSession.id);
    });
  });
  ...
  describe('createSession', () => {
    it('Should create new session by id', async () => {
      ...
      expect(result).toEqual(mockInitSession);
      expect(sessionProvider.createSession).toHaveBeenCalledWith(mockInitSession);
    });
  });
  ...
  describe('updateSessionData', () => {
    it('Should not affect existing data by id', async () => {
      ...
      expect(result).toEqual(mockInitSession);
      expect(sessionProvider.updateSessionData).toHaveBeenCalledWith(mockInitSession.id, {});
    });
    it('Should update data by id', async () => {
      const result = await service.updateSessionData(mockInitSession.id, mockSessionCustomData);

      expect(result).toEqual(mockInitSession);
      expect(sessionProvider.updateSessionData).toHaveBeenCalledWith(mockInitSession.id, mockSessionCustomData);
    });
  ...
  describe('deleteSession', () => {
    it('should delete session by id', async () => {
      ...
      expect(sessionProvider.deleteSession).toHaveBeenCalledWith(mockInitSession.id);
    });
  });
});

...
describe('SshTunnelProvider', () => {
  ...
  describe('createTunnel', () => {
    it('should create tunnel', (done) => {
      service.createTunnel(mockDatabaseWithSshBasic, mockDatabaseWithSshBasic.sshOptions)
        .then((tnl) => {
    ...
    it('should fail due to server init error', (done) => {
      tunnelSshSpy.mockReset().mockImplementationOnce(() => { throw new Error('bb'); });
    ...
    it('should fail with error but not with unable to get property from "undefined"', (done) => {
      ...
      service.createTunnel(undefined, undefined)
        .catch((e) => {
          expect(e).toBeInstanceOf(UnableToCreateTunnelException);
          done();
        });
    });
    it('should fail due to createServer failed with "Cannot parse privateKey" message', (done) => {

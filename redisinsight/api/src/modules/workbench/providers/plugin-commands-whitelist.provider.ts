...
export class PluginCommandsWhitelistProvider {
   ...
   * @param client
   */
  async determineWhitelistCommandsForDatabase(client: RedisClient): Promise<string[]> {
  ...
  async calculateWhiteListCommands(client: RedisClient): Promise<string[]> {
    ...
    try {
      const availableCommands = await client.call(['command'], { replyEncoding }) as string[][];
      ...
      try {
        const dangerousCommands = await client.call(
          ['acl', 'cat', 'dangerous'], { replyEncoding },
        ) as string[];
      ...
      try {
        const blockingCommands = await client.call(
          ['acl', 'cat', 'blocking'], { replyEncoding },
        ) as string[];

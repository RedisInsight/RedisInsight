    ...
    before(async () => {
      ...
      it('should import tutorial from file and generate _manifest.json', async () => {
        ...
        await validateApiCall({
          ...
          fields: [
            ['name', 'Tutorial without manifest'],
          ],
          statusCode: 201,
          checkFn: async ({ body }) => {
            const tutorialRootManifest = {
              type: 'group',
              id: body.id,
              label: 'Tutorial without manifest',
              _actions: [ 'delete' ],
              _path: `/${body.id}`,
              children: autoGeneratedManifest,
            };
        ...
        await validateApiCall({
          ...
          checkFn: async ({ body }) => {
            const tutorialRootManifest = {
              type: 'group',
              id: body.id,
              label: 'Tutorial with manifest',
              _actions: [ 'delete' ],
              _path: `/${body.id}`,
              children: testManifest,
            };
            ...
            expect(body).deep.eq(tutorialRootManifest);
            checkFilesUnarchivedFiles(zip, body?._path);
            expect(JSON.parse(await fsExtra.readFile(path.join(customTutorialsFolder, body._path, 'manifest.json'), 'utf8')))
              .deep.eq(body.children);
      ...
      it('should delete tutorial', async () => {
        ...
        await validateApiCall({
          endpoint: manifestEndpoint,
          checkFn: async ({ body }) => {
            expect(body[0].children.length).eq(3);
          },
        });
        ...
        await validateApiCall({
          endpoint: manifestEndpoint,
          checkFn: async ({ body }) => {
            expect(body[0].children.length).eq(2);
            expect(body).deep.eq([globalManifest]);
          },
        });
      });
      ...
      it('should delete tutorial and not fail even if folder does not exist', async () => {
        ...
        await validateApiCall({
          endpoint: manifestEndpoint,
          checkFn: async ({ body }) => {
            expect(body[0].children.length).eq(1);
            expect(body).deep.eq([globalManifest]);
          },
        });
      });

...
const endpoint = (
  ...
  request(server).delete(`/${constants.API.DATABASES}/${instanceId}/recommendations`);
    ...
    [
      {
        ...
        before: async () => {
          expect(
            await repo.createQueryBuilder().where({ id: constants.TEST_RECOMMENDATION_ID_1 }).getOne()
          ).to.be.an('object');
          expect(
            await repo.createQueryBuilder().where({ id: constants.TEST_RECOMMENDATION_ID_3 }).getOne()
          ).to.be.an('object');
        },
        after: async () => {
          expect(

...
const endpoint = (id: string) => request(server).get(`/${constants.API.RDI}/${id || testRdiId}/pipeline/strategies`);
  ...
  [
    {
      ...
      before: async () => {
        ...
        nock(testRdiUrl).get(`/${RdiUrl.GetStrategies}`).query(true).reply(200, mockResponseSuccess);
      },
    },
    {

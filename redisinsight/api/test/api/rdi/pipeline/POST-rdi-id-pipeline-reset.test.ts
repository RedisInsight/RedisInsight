...
const endpoint = (id) => request(server).post(`/${constants.API.RDI}/${id || testRdiId}/pipeline/reset`);
  ...
  [
    {
      ...
      before: async () => {
        ...
        nock(testRdiUrl).post(`/${RdiUrl.ResetPipeline}`).query(true).reply(200, mockResponse);
    ...
    },
    {
      ...
      before: async () => {
        ...
        nock(testRdiUrl).post(`/${RdiUrl.ResetPipeline}`).query(true).reply(401, {
          message: errorMessage,
          detail: errorMessage,
        });
      },

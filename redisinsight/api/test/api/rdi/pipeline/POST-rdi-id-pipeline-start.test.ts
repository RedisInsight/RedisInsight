...
const endpoint = (id) => request(server).post(`/${constants.API.RDI}/${id || testRdiId}/pipeline/start`);
  ...
  [
    {
      ...
      before: async () => {
        ...
        nock(testRdiUrl).post(`/${RdiUrl.StartPipeline}`).query(true).reply(200, mockResponse);
    ...
    },
    {
      ...
      before: async () => {
        ...
        nock(testRdiUrl).post(`/${RdiUrl.StartPipeline}`).query(true).reply(401, {
          message: errorMessage,
          detail: errorMessage,
        });
      },

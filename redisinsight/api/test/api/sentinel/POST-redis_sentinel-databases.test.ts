    ...
    it('Create sentinel database (tls)', async () => {
      ...
      await validateApiCall({
        ...
        checkFn: async ({ body }) => {
          ...
          expect(db.password).to.eql(localDb.encryptData(constants.TEST_REDIS_PASSWORD));
          expect(db.sentinelMasterPassword).to.eql(localDb.encryptData(constants.TEST_SENTINEL_MASTER_PASS));
          expect(db.caCert.certificate).to.eql(localDb.encryptData(constants.TEST_REDIS_TLS_CA));
          expect(db.clientCert.certificate).to.eql(localDb.encryptData(constants.TEST_USER_TLS_CERT));
          expect(db.clientCert.key).to.eql(localDb.encryptData(constants.TEST_USER_TLS_KEY));
        },
      });
    });
    it('Create sentinel database with plain auth data', async () => {
      ...
      await validateApiCall({
        ...
        checkFn: async ({ body }) => {
          ...
          expect(db.clientCert.certificate).to.eql(constants.TEST_USER_TLS_CERT);

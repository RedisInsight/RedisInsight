...
const endpoint = (instanceId = constants.TEST_INSTANCE_ID) =>
  request(server).get(`/${constants.API.DATABASES}/${instanceId}/triggered-functions/functions`);

const responseSchema = Joi.array().items(Joi.object({
  type: Joi.string().valid('functions', 'cluster_functions', 'keyspace_triggers', 'stream_triggers').required(),

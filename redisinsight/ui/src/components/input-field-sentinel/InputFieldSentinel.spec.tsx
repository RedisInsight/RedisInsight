...
import InputFieldSentinel, { Props, SentinelInputFieldType } from './InputFieldSentinel'
...
describe('InputFieldSentinel', () => {
  ...
  it('should change simple fieldText properly', () => {
    render(<InputFieldSentinel {...instance(mockedProps)} inputType={SentinelInputFieldType.Text} />)
    fireEvent.change(screen.getByTestId(inputTextTestId), { target: { value: 'val' } })
    expect(screen.getByTestId(inputTextTestId)).toHaveValue('val')
  })

  it('should render Password field', () => {
    render(<InputFieldSentinel {...instance(mockedProps)} inputType={SentinelInputFieldType.Password} />)
    expect(screen.getByTestId(inputPasswordTestId)).toBeInTheDocument()
  })

  it('should change Password field properly', () => {
    render(<InputFieldSentinel {...instance(mockedProps)} inputType={SentinelInputFieldType.Password} />)
    fireEvent.change(screen.getByTestId(inputPasswordTestId), { target: { value: 'val' } })
    expect(screen.getByTestId(inputPasswordTestId)).toHaveValue('val')
  })

  it('should render Number field', () => {
    render(<InputFieldSentinel {...instance(mockedProps)} inputType={SentinelInputFieldType.Number} />)
    expect(screen.getByTestId(inputNumberTestId)).toBeInTheDocument()
  })

  it('should change Number field properly', () => {
    render(<InputFieldSentinel {...instance(mockedProps)} inputType={SentinelInputFieldType.Number} />)
    fireEvent.change(screen.getByTestId(inputNumberTestId), { target: { value: 'val13' } })

...
import { cleanup, mockedStore, render, initialStateDefault, mockStore } from 'uiSrc/utils/test-utils'
...
const initialMockUser: CloudUser = {
  ...
  currentAccountId: 45,
  accounts: [
    { id: 45, name: "Account 1" },
    { id: 46, name: "Account 2" },
  ],
  data: {}
}

type MockStoreStateProps = {
  envDependant?: boolean;
  cloudSso?: boolean;
  cloudAds?: boolean;
  mockUser?: CloudUser;
}
...
const mockStoreStateWithFlags = ({
  ...
  const values = [envDependant, cloudSso, cloudAds]
  ...
  set(initialStoreState, 'user.cloudProfile', { error: '', data: mockUser })
  set(initialStoreState, 'oauth.cloud.user', { error: '', loading: false, initialLoading: false, data: mockUser })
...
describe('UserProfile', () => {
  ...
  it('should render CloudUserProfile if envDependentFeature is disabled', () => {
    store = mockStoreStateWithFlags({ envDependant: false})
  ...
  it('should not show cloud user profile badge profile name is empty', () => {
    store = mockStoreStateWithFlags({ envDependant: false, mockUser: { ...initialMockUser, name: '' } })
  ...
  it('should render nothing when envDependent=true, cloudAds=true, cloudSso=false', () => {
    store = mockStoreStateWithFlags({ envDependant: true, cloudAds: true, cloudSso: false })
    const { queryByTestId } = render(<UserProfile />, {
      store
    })

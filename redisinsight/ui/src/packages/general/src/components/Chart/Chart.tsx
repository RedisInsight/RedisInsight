...
export default function Chart(props: ChartProps) {
  ...
  useEffect(() => {
    ...
    chartContainer.current.on(PlotlyEvents.PLOTLY_HOVER, (eventdata: PlotMouseEvent) => {
    ...
    chartContainer.current.on(PlotlyEvents.PLOTLY_RELAYOUT, (eventdata: PlotRelayoutEvent) => {
  ...
  function getData(props: ChartProps): Partial<PlotData>[] {
    return props.data.map((timeSeries, i) => {
      ...
      const x = dataUnchanged
        ? currentData[i].x
        : selectCol(timeSeries.datapoints, 0)
          .map((time: number) => format(time, 'yyyy-MM-dd HH:mm:ss.SSS'))

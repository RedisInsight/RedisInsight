...
function GraphD3(_selector: HTMLDivElement, _options: any): IGraphD3 {
  ...
  const zoom: d3.ZoomBehavior<Element, unknown> = options.graphZoom
  ...
  function appendTextToNode(svgNode, textSize = null) {
    svgNode
      ...
      .data((d) => {
        ...
        label ||= ''
        if (textSize !== null) {
          const maxLength = (maxTextSize - textSize) + 15
          label = label.length > maxLength ? Utils.truncateText(label, maxLength) : label
        }
        const wrappedLabel = Utils.wrapText(label, 7)
  ...
  function updateRelationships(r: IRelationship) {
    ...
    const a = svgRelationships.selectAll('.relationship')
  ...
  function updateNodesAndRelationships(n, r) {
    ...
    const edgeIds = relationships.map((e) => e.id)
  ...
  function tickRelationshipsOutlines() {
    relationship.each(function (relationship) {
      ...
      const outline = rel.select('.outline') as unknown as d3.Selection<d3.BaseType, IRelationship, null, undefined>
  ...
  function tickRelationshipsOverlays() {
    relationshipOverlay.attr('d', (d) => d.arrow.overlay(options.arrowSize))
  }
  ...
  function tickRelationships() {
    ...
    if (relationship) {
      ...
      relationship.attr('transform', (d) => `translate(${d.source.x} ${d.source.y}) rotate(${
        d.naturalAngle + 180
      })`)
      ...
      for (const nodePair of Array.from(nodePairs)) {
        if (nodePair.isLoop()) {
          ...
          if (angles.length > 0) {
            ...
            separation = (biggestGap.end - biggestGap.start)

...
export default function Explain(props: ResponseProps): JSX.Element {
  ...
  if (command === 'ft.profile') {
    ...
    if (info.length > 5 && typeof info[0] === 'string' && info[0].toLowerCase().startsWith('shard')) {
      const [cluster, entityInfo] = ParseProfileCluster(info)
    ...
    } if (typeof info[0] === 'string' && info[0].toLowerCase().startsWith('coordinator')) {

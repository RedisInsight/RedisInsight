...
describe('ConsumersView', () => {
  ...
  it('should render default message when no consumers and custom message is not specified', () => {
    ...
    expect(screen.getByTestId('stream-consumers-container'))
      .toHaveTextContent('Your Consumer Group has no Consumers available.')
  })

  it('should render custom message when no consumers and custom message is specified', () => {
    render(<ConsumersView {...instance(mockedProps)} data={[]} noItemsMessageString="customNoItemsMessageString" />)
    expect(screen.getByTestId('stream-consumers-container')).toHaveTextContent('customNoItemsMessageString')
  })
})

...
import DatabasesList from '../item-list-component'
...
const Databases = ({ resizeRef, width }: Props) => {
  const [openRightPanel, setOpenRightPanel] = useState<Nullable<RightPanelName>>(null)
  const [welcomeIsShow, setWelcomeIsShow] = useState(!localStorageService.get(BrowserStorageItem.instancesCount))
  ...
  useEffect(() => {
    const instancesCashCount = JSON.parse(localStorageService.get(BrowserStorageItem.instancesCount) ?? '0')
  ...
  if (welcomeIsShow) {
    return <WelcomeComponent onAddInstance={handleAddInstance} />
  }

  return (
    <>
      <HomeHeader key="instance-controls" onAddInstance={handleAddInstance} direction="row" />
      {openRightPanel && instances.length ? (
        <div key="homePage" className="homePage">
          <EuiResizableContainer style={{ height: '100%' }}>
            {(EuiResizablePanel, EuiResizableButton) => (
              <>
                <EuiResizablePanel scrollable={false} initialSize={62} id="databases" minSize="50%" paddingSize="none">
                  <div ref={resizeRef}>
                    <DatabasesList
                      width={width}
                ...
                >
                  {!!openRightPanel && (
                    <DatabasePanel
                      ...
                      editedInstance={
                        openRightPanel === RightPanelName.EditDatabase ? editedInstance : sentinelInstance ?? null
                      }
                      onClose={openRightPanel === RightPanelName.EditDatabase ? closeEditDialog : handleClose}

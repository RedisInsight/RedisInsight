...
import { render, screen, fireEvent, mockedStore, cleanup, initialStateDefault } from 'uiSrc/utils/test-utils'
...
describe('Navigation', () => {
  ...
  it('should not show nav when pipeline is loading', () => {
    ...
    expect(screen.queryByTestId(`rdi-nav-btn-${RdiPipelineTabs.Config}`)).not.toBeInTheDocument()
  })
  ...
  it('should show nav when pipeline is not loading', () => {
    ...
    expect(screen.queryByTestId(`rdi-nav-btn-${RdiPipelineTabs.Config}`)).toBeInTheDocument()
  })

...
import {
  findStopArgument,
  findStopArgumentWithSuggestions,
  findSuggestionsByQueryArgs
} from 'uiSrc/pages/workbench/utils/query_refactor'
import {
  ...
  describe('FT.AGGREGATE', () => {
    findArgumentftAggreageTests.forEach(({ args, result: testResult }) => {
      it(`should return proper suggestions for ${args.join(' ')}`, () => {
        const result = findStopArgumentWithSuggestions({
          queryArgs: args,
          command: ftAggregateCommand
        })
        expect(testResult?.stopArg).toEqual(result.stopArg)
      })
    })
  })

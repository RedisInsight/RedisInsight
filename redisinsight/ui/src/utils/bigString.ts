...
import { RedisResponseBuffer, RedisResponseBufferType, RedisString } from 'uiSrc/slices/interfaces'
import { Nullable } from 'uiSrc/utils/types'

const BIG_STRING_PREFIX = getConfig().app.truncatedStringPrefix
const BIG_STRING_PREFIX_BUFFER = Uint8Array.from(Array.from(BIG_STRING_PREFIX).map((letter) => letter.charCodeAt(0)))

const redisResponseBufferStartsWith = (value: RedisResponseBuffer, subBuffer: Uint8Array) => {

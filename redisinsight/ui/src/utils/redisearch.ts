...
import { REDISEARCH_GEOSHAPE_VERSION } from 'uiSrc/constants'
...
const isGeoshapeOptionAvailable = (modules: AdditionalRedisModule[]): boolean =>
  modules?.some(({ name, semanticVersion }) =>
    REDISEARCH_MODULES
      .some((search) => name === search && isVersionHigherOrEquals(semanticVersion, REDISEARCH_GEOSHAPE_VERSION)))

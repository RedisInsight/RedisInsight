...
test
    .before(async () => {
        await acceptLicenseTermsAndAddDatabaseApi(ossStandaloneConfig, ossStandaloneConfig.databaseName);
        // Create new keys
        await addKeysViaCli(keysData, formatters[0].fromText);
...
test('Verify that user can edit the values in the key regardless if they are valid in JSON/Msgpack format or not', async t => {
    ...
    for (let formatter of formatters) {
        if (formatter.format === "JSON" || formatter.format === "Msgpack") {
            ...
            await t.expect(browserPage.hashFieldValue.textContent).contains(invalidText, `Invalid ${formatter} value is not saved`);
            await t.click(browserPage.editHashButton);
            ...
            await t.expect(browserPage.hashFieldValue.innerText).contains(formatter.fromText, `Invalid ${formatter} value is not saved`);
            await t.expect(browserPage.hashFieldValue.find(browserPage.cssJsonValue).exists).ok(`Value is not formatted to ${formatter}`);
        }
    }
});
        ...
        for (let formatter of formatters) {
            if (formatter.format === "ASCII" || formatter.format === "HEX") {
                ...
                for (let i = 0; i < keysData.length; i++) {
                    ...
                    await t.expect(valueSelector.innerText).contains(formatter.fromText!, 'Value is not saved as msgpack');
                    await browserPage.selectFormatter(formatter.format);
                    // Verify that value is formatted and highlighted
                    await t.expect(valueSelector.innerText).contains(formatter.toText!, 'Value is not saved as msgpack');
                }
            }
        }
    });

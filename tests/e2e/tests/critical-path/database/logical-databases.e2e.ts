...
test('Verify that user can add DB with logical index via host and port from Add DB manually form', async t => {
    ...
    await t.typeText(myRedisDatabasePage.AddRedisDatabase.databaseIndexInput, index, { replace: true, paste: true });
    ...
    await t.expect(myRedisDatabasePage.AddRedisDatabase.databaseIndexMessage.exists).notOk('Index message is still displayed')
        .expect(myRedisDatabasePage.AddRedisDatabase.databaseIndexCheckbox.parent().withExactText('Select Logical Database').exists).ok('Checkbox text not displayed');
    // Click for saving

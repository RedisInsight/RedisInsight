...
// READY
test('Verify that every time when notification manager publish notification.json file to latest, new message is displayed in the application', async t => {
...
test.only('Verify that user can open notification center by clicking on icon and see all received messages', async t => {
    // Verify that user can see number of new notifications on notification center icon
    await t.expect(await notificationPage.getUnreadNotificationNumber()).eql(29, 'Number of unread messages in badge');
    ...
    await t.expect(notificationPage.unreadNotification.count).eql(29, 'Highlighted messages');

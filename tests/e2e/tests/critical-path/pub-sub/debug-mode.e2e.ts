...
fixture `PubSub debug mode`
    ...
    .beforeEach(async t => {
        ...
        //Go to PubSub page and subscribe to chanel
...
test
    .meta({ env: env.web, rte: rte.standalone })('Verify that when user navigating away and back to pubsub window the debug mode state will be reset to default auto-scroll', async t => {
        ...
        await t.expect(pubSubPage.pubSubPageContainer.find(pubSubPage.cssSelectorMessage).withText('new message to with no scroll').visible).ok('Message is not visible');
    });
test
    .meta({ env: env.web, rte: rte.standalone })('Verify that when user scroll all the way to the newest available message (down), auto-scroll resumes automatically.', async t => {
        //Scroll to the first messages
        await t.scrollIntoView(pubSubPage.pubSubPageContainer.find(pubSubPage.cssSelectorMessage).withText('first'));
...
test
    .meta({ env: env.web, rte: rte.standalone })('Verify that user can get to the newest message in one click', async t => {
        //Scroll to the first messages
        await t.scrollIntoView(pubSubPage.pubSubPageContainer.find(pubSubPage.cssSelectorMessage).withText('first'));

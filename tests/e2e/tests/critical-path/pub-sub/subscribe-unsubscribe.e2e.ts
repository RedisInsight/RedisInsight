...
test
    .meta({ rte: rte.standalone })('Verify that when user subscribe to the pubsub channel he can see all the messages being published to my database from the moment of my subscription', async t => {
        ...
        await t.expect(pubSubPage.pubSubPageContainer.find('[data-testid^=row]').withText('message').exists).ok(`Message is not displayed`);
        ...
        //Unsubscribe from chanel and publish message
        ...
        await pubSubPage.publishMessage('test', 'message2');
        //Verify that message is not displayed
        await t.expect(pubSubPage.pubSubPageContainer.find('[data-testid^=row]').withText('message2').exists).notOk(`Message is displayed`);
    });
test
    .meta({ rte: rte.standalone })('Verify that my subscription state is preserved when user navigate through the app while connected to current database and in current app session', async t => {
...
test
    .meta({ rte: rte.standalone })('Verify that the focus gets always shifted to a newest message (auto-scroll)', async t => {
        await pubSubPage.subsribeToChanelAndPublishMessage('test', 'message');
        //Publish 100 messages
        await cliPage.sendCommandInCli(`100 publish chanel test`);
        //Verify that the first message is not visible in view port
        await t.expect(pubSubPage.pubSubPageContainer.find('[data-testid^=row]').withText('message').visible).notOk(`Message is visible`);
        await t.expect(pubSubPage.pubSubPageContainer.find('[data-testid^=row]').withText('test').visible).ok(`Message is not displayed`);
    });
test
    .before(async t => {
    ...
    .meta({ rte: rte.standalone })('Verify that user subscription state is changed to unsubscribed, all the messages are cleared and total message counter is reset when user connect to another database)', async t => {
        await t.click(pubSubPage.subscribeButton);
        //Publish 10 messages
        ...
        await t.click(myRedisDatabasePage.pubSubButton);
        await t.expect(pubSubPage.subscribeStatus.textContent).eql('You are not subscribed', 'User is not unsubscribed', { timeout: 10000 });
        await t.expect(pubSubPage.pubSubPageContainer.find('[data-testid^=row]').withText('message').exists).notOk(`Message is displayed`);
        await t.expect(pubSubPage.totalMessagesCount.exists).notOk('Total counter is still displayed');
    });
test
    .meta({ rte: rte.standalone })('Verify that user can see a internal link to pubsub window under word “Pub/Sub” when he try to run PSUBSCRIBE command in CLI or Workbench', async t => {
        //Go to Browser Page
...
test
    .meta({ rte: rte.standalone })('Verify that when user click Publish and the publication is successful, he can see a response: badge with the number <# of clients received>', async t => {
        await pubSubPage.subsribeToChanelAndPublishMessage('test', 'message');
...
test
    .meta({ rte: rte.standalone })('Verify that the Message field input is preserved until user Publish a message', async t => {
        //Fill in Channel and Message inputs
        await t.click(pubSubPage.subscribeButton);
        await t.click(pubSubPage.channelNameInput);
        await t.typeText(pubSubPage.channelNameInput, 'test', { replace: true });
        ...
        await t.expect(pubSubPage.channelNameInput.value).eql('test', 'Channel input is empty', { timeout: 10000 });
    });
test
    .meta({ rte: rte.standalone })('Verify that user can clear all the messages from the pubsub window', async t => {
        await pubSubPage.subsribeToChanelAndPublishMessage('test', 'message');
        await pubSubPage.publishMessage('test', 'second m');
        ...
        await t.expect(pubSubPage.pubSubPageContainer.find('[data-testid^=row]').withText('message').exists).notOk(`Message is still displayed`);
        await t.expect(pubSubPage.pubSubPageContainer.find('[data-testid^=row]').withText('second m').exists).notOk(`Second message is still displayed`);
    });

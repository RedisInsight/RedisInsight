...
test
    .meta({ env: env.web, rte: rte.standalone })('Verify that when user subscribe to the pubsub channel he can see all the messages being published to my database from the moment of my subscription', async t => {
        ...
        await t.expect(pubSubPage.pubSubPageContainer.find(pubSubPage.cssSelectorMessage).withText('published message').exists).ok('Message is not displayed');
...
test
    .meta({ env: env.web, rte: rte.standalone })('Verify that user can clear all the messages from the pubsub window', async t => {
        ...
        await t.expect(pubSubPage.pubSubPageContainer.find(pubSubPage.cssSelectorMessage).withText('message').exists).notOk('Message is still displayed');
        await t.expect(pubSubPage.pubSubPageContainer.find(pubSubPage.cssSelectorMessage).withText('second m').exists).notOk('Second message is still displayed');
    });

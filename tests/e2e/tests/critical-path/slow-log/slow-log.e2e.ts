...
test
    .meta({ rte: rte.none })('Verify that user can open new Slow Log page using new icon on left app panel', async t => {
        ...
        await slowLogPage.compareCommandExecutionWithSlowerThan(command, slowerThanParameter);
...
test
    .meta({ rte: rte.none })('Verify that user can see "No Slow Logs found" message when slowlog-max-len=0', async t => {
...
test
    .meta({ rte: rte.none })('Verify that user can clear Slow Log', async t => {
        // Set slowlog-max-len=0
        command = 'info';
        await slowLogPage.changeSlowerThanParameter(slowerThanParameter);
...
test
    .meta({ rte: rte.none })('Verify that users can specify number of commands that they want to display (10, 25, 50, 100, Max) in Slow Log', async t => {
        ...
        for (let i = 0; i < numberOfCommandsArray.length; i++) {
            ...
            if (i === numberOfCommandsArray.length - 1) {
                ...
                await t.expect(slowLogPage.slowLogCommandStatistics.withText(`${numberOfCommandsArray[i]} entries`).exists).ok('Number of displayed commands is less than ');
            }
        }
    });
test
    .meta({ rte: rte.none })('Verify that user can set slowlog-log-slower-than value in miliseconds and command duration will be re-calculated to ms', async t => {
        ...
        // console.log(`recalculatedDuration: ${microsecondsDuration}`);
        // console.log(`duration parseFloat / 1000: ${parseFloat(millisecondsDration.replace(' ', '')) / 1000}`);
        // console.log(`recalculatedDuration parseFloat: ${parseFloat(microsecondsDuration.replace(' ', ''))}`);

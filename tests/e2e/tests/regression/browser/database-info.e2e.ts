...
test
    .meta({ rte: rte.standalone })
    ('Verify that user can see DB name, endpooint, connection type, Redis version, user name in tooltip when hover over the (i) icon', async t => {
        ...
        await t.expect(browserPage.databaseInfoToolTip.textContent).contains(`${ossStandaloneConfig.host}:${ossStandaloneConfig.port}`, 'User can see endpooint in tooltip');

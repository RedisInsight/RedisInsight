...
test('Verify that user can see not available metrics from Overview in tooltip with the text "<Metric_name> is/are not available"', async t => {
    ...
    await t.expect(databaseOverviewPage.overviewTooltip.exists).ok('Overview tooltip not displayed');
    // Verify that Database statistics title is displayed in tooltip
    await t.expect(databaseOverviewPage.overviewTooltipStatTitle.exists).ok('Statistics title not displayed');

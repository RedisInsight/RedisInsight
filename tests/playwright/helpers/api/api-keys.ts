...
const databaseAPIRequests = new DatabaseAPIRequests(apiUrl)
...
export class APIKeyRequests {
    ...
    async addHashKeyApi(keyParameters: HashKeyParameters, databaseParameters: AddNewDatabaseParameters): Promise<void> {
        ...
        const response = await apiClient.post(`/databases/${databaseId}/hash?encoding=buffer`, requestBody)
    ...
    async addStreamKeyApi(keyParameters: StreamKeyParameters, databaseParameters: AddNewDatabaseParameters): Promise<void> {
        ...
        const response = await apiClient.post(`/databases/${databaseId}/streams?encoding=buffer`, requestBody)
    ...
    async addSetKeyApi(keyParameters: SetKeyParameters, databaseParameters: AddNewDatabaseParameters): Promise<void> {
        ...
        const response = await apiClient.post(`/databases/${databaseId}/set?encoding=buffer`, requestBody)
    ...
    async searchKeyByNameApi(keyName: string, databaseName: string): Promise<string[]> {
        ...
        const response = await apiClient.post(bufferPathMask.replace('databaseId', databaseId), requestBody)
    ...
    async deleteKeyByNameApi(keyName: string, databaseName: string): Promise<void> {
        ...
        if (doesKeyExist.length > 0) {
            const requestBody = { keyNames: [Buffer.from(keyName, 'utf-8')] }
            const response = await apiClient.delete(bufferPathMask.replace('databaseId', databaseId), { data: requestBody })

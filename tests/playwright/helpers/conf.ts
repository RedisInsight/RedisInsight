...
export const ossStandaloneTlsConfig = {
    ...
    caCert: {
        name: `ca}-${uniqueId}`,
        certificate: process.env.E2E_CA_CRT || fs.readFileSync(path.resolve(__dirname, '../local-docker-environment/rte/oss-standalone-tls/certs/redisCA.crt'), 'utf-8')
    },
    clientCert: {
        name: `client}-${uniqueId}`,
        certificate: process.env.E2E_CLIENT_CRT || fs.readFileSync(path.resolve(__dirname, '../local-docker-environment/rte/oss-standalone-tls/certs/redis.crt'), 'utf-8'),
        key: process.env.E2E_CLIENT_KEY || fs.readFileSync(path.resolve(__dirname, '../local-docker-environment/rte/oss-standalone-tls/certs/redis.key'), 'utf-8')
    }
}
